function  cutData(filePath,savePath)
...
% Sternberg block 7
i37 = find(ismember({EEGorig.event.type}, '37'));
i47 = find(ismember({EEGorig.event.type}, '47'));

% Sternberg block 8
i38 = find(ismember({EEGorig.event.type}, '38'));
i48 = find(ismember({EEGorig.event.type}, '48'));
...
if not(isempty(i10))
    ...
    save(fullfile(savePath, task), 'EEG', '-v7.3')
end
...
if not(isempty(i37))
    EEG = pop_select(EEGorig, 'point', [EEGorig.event(i37(end)).latency, EEGorig.event(i47(end)).latency]);
    task = [num2str(subjectID), '_OCC_Sternberg', '_block7', '_task_EEG.mat'];
    %     % photodiode
    %     EEG.Photo = pop_select(EEG, 'channel', [129 : 152]);
    %     EEG = pop_select(EEG, 'nochannel', [129 : 152]);
    save(fullfile(savePath, task), 'EEG', '-v7.3')
...
if not(isempty(i38))
    EEG = pop_select(EEGorig, 'point', [EEGorig.event(i38(end)).latency, EEGorig.event(i48(end)).latency]);
    task = [num2str(subjectID), '_OCC_Sternberg', '_block8', '_task_EEG.mat'];
    %     % photodiode
    %     EEG.Photo = pop_select(EEG, 'channel', [129 : 152]);
    %     EEG = pop_select(EEG, 'nochannel', [129 : 152]);
    save(fullfile(savePath, task), 'EEG', '-v7.3')
