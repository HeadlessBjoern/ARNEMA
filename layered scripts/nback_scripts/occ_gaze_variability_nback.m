%% Gaze Variability for N-BACK TASK
...
for subj = 1:length(subjects)
    ...
    for condition = 1:3
        ...
        distances_mean = sqrt((x_positions - mean_x).^2 + (y_positions - mean_y).^2);

        % Calculate and store the standard deviation for this condition
        condition_index = condition; % Directly use condition as index
    ...
    save([datapath, filesep, 'gaze_std_devs_nback.mat'], 'std_devs_per_condition_mean');
...
for subs = 1:length(subjects)
    ...
    file_to_load = fullfile(base_path, subject_folder, 'gaze_std_devs_nback.mat');
...
stdDevsTable = array2table(stdDevsData, 'VariableNames', {'Condition1', 'Condition2', 'Condition3'});
...
colormap([0 0 1; 0 1 0; 1 0 0]); % Blue, Green, Red
...
title('Standard Deviations for Each Subject and N-back Condition', 'FontSize', 15);
...
legend('1-back', '2-back', '3-back', 'FontSize', 15);
...
for i = 1:numConditions
    for j = i+1:numConditions
        ...
        comparisonLabels{counter} = sprintf('%d-back vs. %d-back', i, j);
...
for i = 1:numConditions
    for j = i+1:numConditions
        ...
        if p < max(sigLevels)
            ...
            vLineLength = yOffset * 0.4; % Adjust the length as needed

            % Draw horizontal line
            line([i, j], [yPos, yPos], 'Color', 'k', 'LineWidt', 2);

            % Draw vertical lines at the ends of the horizontal line
            bracketPos = yPos-7;
            line([i, i], [bracketPos - vLineLength, bracketPos + vLineLength], 'Color', 'k', 'LineWidt', 2); % Left vertical line
            line([j, j], [bracketPos - vLineLength, bracketPos + vLineLength], 'Color', 'k', 'LineWidt', 2); % Right vertical line
...
set(gca, 'xtick', 1:numConditions, 'xticklabel', {'1-back', '2-back', '3-back'}, 'FontSize', 20);
xlabel('Condition', 'FontSize', 30);
...
% Legend
legend([plotHandles{:}], {'1-back', '2-back', '3-back'}, 'FontSize', 20, 'Location', 'northeast');
...
saveas(gcf, '/Volumes/methlab/Students/Arne/MA/figures/gaze/Nback_gaze_variability_boxplots_sign.png');
...
for i = 1:numConditions
    for j = i+1:numConditions
        fprintf('%d-back vs. %d-back: %.2f\n', i, j, cliffsDeltaValues(i,j));
    end
end

%% Boxplots for N-back Task
close all

% Extract the data from the table for N-back task, excluding the overall mean
% Assuming stdDevsDataNback is already calculated and has the same format
stdDevsDataNback = table2array(stdDevsTable(1:end-1, 2:end));

% Create a figure
figure('Color', 'w');
set(gcf, 'Position', [100, 100, 1200, 1000]);

% Number of conditions and subjects for N-back
numConditionsNback = size(stdDevsDataNback, 2);
numSubjectsNback = size(stdDevsDataNback, 1);

% Colors for each N-back condition
colorsNback = [0 0 1; 0 1 0; 1 0 0]; % Blue, Green, Red

% Plotting boxplot and individual data points for N-back
plotHandlesNback = cell(1, numConditionsNback); % To store plot handles for the legend
for i = 1:numConditionsNback
    % Boxplot
    boxplot(stdDevsDataNback(:, i), 'Positions', i, 'Widths', 0.2, 'Colors', colorsNback(i, :), 'Symbol', '');
    hold on;
    
    % Individual data points and store the handle for legend
    plotHandlesNback{i} = plot(NaN, NaN, 'o', 'Color', colorsNback(i, :), 'MarkerSize', 6, 'MarkerFaceColor', colorsNback(i, :));
    
    % Plot individual data points
    for subj = 1:numSubjectsNback
        plot(i, stdDevsDataNback(subj, i), 'o', 'Color', colorsNback(i, :), 'MarkerSize', 6, 'MarkerFaceColor', colorsNback(i, :));
    end
end

% Add median values for each condition in N-back
medianValuesNback = median(stdDevsDataNback);
for i = 1:numConditionsNback
    text(i, medianValuesNback(i)+0.25, num2str(medianValuesNback(i), '%.2f'), 'HorizontalAlignment', 'center', 'VerticalAlignment', 'bottom', 'FontSize', 20, 'Color', 'k');
end

% Connect points from the same participant in N-back
for subj = 1:numSubjectsNback
    plot(1:numConditionsNback, stdDevsDataNback(subj, :), '-o', 'Color', 'k', 'MarkerSize', 6, 'MarkerEdgeColor', 'k');
end

% Assuming numConditionsNback, stdDevsDataNback, and origSigLevels are already defined
numConditions = numConditionsNback;
pValues = zeros(numConditions);
numComparisons = numConditions * (numConditions - 1) / 2; % Total number of comparisons

% Perform Wilcoxon signed-rank tests
for i = 1:numConditions
    for j = i+1:numConditions
        [p, ~] = signrank(stdDevsDataNback(:, i), stdDevsDataNback(:, j));
        pValues(i, j) = p;
    end
end

% Flatten and sort p-values
pValuesVector = pValues(triu(true(numConditions), 1)); % Extract upper triangular part excluding diagonal
[sortedPValues, sortOrder] = sort(pValuesVector);

% Apply Benjamini-Hochberg correction
m = length(sortedPValues); % Total number of comparisons
BHThresholds = (1:m) .* origSigLevels / m; % Adjusted thresholds
adjustedPValues = min(1, cummin(m * sortedPValues ./ (1:m))); % B-H correction

% Re-map corrected p-values back to their original matrix format
adjustedPValuesMatrix = zeros(numConditions);
adjustedPValuesMatrix(triu(true(numConditions), 1)) = adjustedPValues(sortOrder);

% Visualization
sigSymbols = {'*', '**', '***'};
lineCounts = zeros(size(origSigLevels));
maxDataPoint = max(stdDevsDataNback(:));
yOffset = maxDataPoint * 0.1; % Offset for displaying significance symbols

for i = 1:numConditions
    for j = i+1:numConditions
        p = adjustedPValuesMatrix(i, j);
        if p < origSigLevels(1) % You can adjust this as needed
            % Find the appropriate symbol and its index
            symbolIndex = find(p < origSigLevels, 1, 'last');
            symbol = sigSymbols{symbolIndex};

            % Update line count for the current significance level
            lineCounts(symbolIndex) = lineCounts(symbolIndex) + 1;

            % Calculate position for the symbol
            xPos = (i + j) / 2;
            yPos = maxDataPoint + yOffset * (symbolIndex + lineCounts(symbolIndex));
            if i == 1
                yPos = yPos * 0.95;
            elseif i == 3 
                yPos = yPos * 1.05;
...
% Customizing the axes for N-back
set(gca, 'xtick', 1:numConditionsNback, 'xticklabel', {'1-back', '2-back', '3-back'}, 'FontSize', 20);
set(gca, 'YScale', 'log');
...
% Legend for N-back
legend([plotHandlesNback{:}], {'1-back', '2-back', '3-back'}, 'FontSize', 20, 'Location', 'best');

% Improve layout for N-back plot
box on;

% Show the figure for N-back
hold off;

saveas(gcf, '/Volumes/methlab/Students/Arne/MA/figures/gaze/Nback_gaze_variability_boxplots.png');

%% Relation with EEG alpha differences (RELATIVE VALUES) for N-back Task
clc;
close all;
clc;
close all;
...
for subj = 1:length(subjects)
    ...
    if exist('IAF_nback.mat', 'file')
        load('IAF_nback.mat', 'powerIAF1', 'powerIAF3');
        
        % Assuming powerIAF2 and powerIAF8 are arrays with one element per subject
        peakPowers1 = [peakPowers1, powerIAF1(subj)];
        peakPowers3 = [peakPowers3, powerIAF3(subj)];
    else
        fprintf('IAF.mat not found for subject %s\n', subjects{subj});
    end
end
numParticipantsNback = length(peakPowers1);

% Initialize arrays to store percentage changes for each participant
deltaAlphaNbackPercent = zeros(numParticipantsNback, 1);
deltaStdGazeNbackPercent = zeros(numParticipantsNback, 1);

% Loop through each participant
for i = 1:numParticipantsNback
    % Calculate the percentage change in alpha peak power for each participant
    deltaAlphaNbackPercent(i) = ((peakPowers3(i) - peakPowers1(i)) / abs(peakPowers1(i))) * 100;

    % Calculate the percentage change in standard deviation of gaze for each participant
    deltaStdGazeNbackPercent(i) = ((stdDevsDataNback(i, 3) - stdDevsDataNback(i, 1)) / abs(stdDevsDataNback(i, 1))) * 100;
end

% Create a scatter plot for N-back with percentage changes
figure('Color','w');
set(gcf, "Position", [100, 100, 1200, 1000])
scatter(deltaStdGazeNbackPercent, deltaAlphaNbackPercent, 100, 'filled', 'MarkerFaceColor', 'k');
...
maxAbsX = max(abs(deltaStdGazeNbackPercent));
maxAbsY = max(abs(deltaAlphaNbackPercent));
...
saveas(gcf, '/Volumes/methlab/Students/Arne/MA/figures/gaze/Nback_gaze_variability_fit.png');

%% Calculate basic statistics for deltaAlpha and deltaStdGaze
clc

deltaStdGaze = deltaStdGazeNbackPercent;
deltaAlpha = deltaAlphaNbackPercent;
...
%% Relation with EEG alpha differences for N-back (DELTA Alpha vs. DELTA ET)
clc;
close all;
clc;
close all;
...
% Initialize arrays to store changes for each participant in N-back
deltaAlphaNback = zeros(numParticipantsNback, 1);
deltaStdGazeNback = zeros(numParticipantsNback, 1);

% Loop through each participant for N-back
for i = 1:numParticipantsNback
    % Calculate the change in alpha peak power for each participant in N-back
    deltaAlphaNback(i) = peakPowers3(i) - peakPowers1(i);

    % Calculate the change in standard deviation of gaze for each participant in N-back
    deltaStdGazeNback(i) = stdDevsDataNback(i, 3) - stdDevsDataNback(i, 1);
end

% Create a scatter plot for N-back with absolute changes
figure('Color','w');
set(gcf, "Position", [100, 100, 1200, 1000])
scatter(deltaStdGazeNback, deltaAlphaNback, 'filled');
xlabel('\Delta Std Deviation of Gaze (3-back - 1-back) [%]', 'FontSize', 20);
ylabel('\Delta Alpha Peak Power (3-back - 1-back) [%]', 'FontSize', 20);
title('');

% Add a line of best fit for N-back
hold on;
pNback = polyfit(deltaStdGazeNback, deltaAlphaNback, 1); % Linear fit for N-back
xFitNback = linspace(min(deltaStdGazeNback), max(deltaStdGazeNback), 100);
yFitNback = polyval(pNback, xFitNback);
plot(xFitNback, yFitNback, '-r', 'LineWidth', 2);

% Add grid and improve layout for N-back plot
box on;

% Show the plot for N-back
hold off;

saveas(gcf, '/Volumes/methlab/Students/Arne/MA/figures/eeg/Nback_gaze_variability_fit_absolutediffs.png');
...
for i = 1:numParticipants
    ...
    hold on;
    scatter(stdDevsData(i, 3), peakPowers3(i), 'filled', 'MarkerFaceColor', 'r'); % Condition 2
    
    % Draw a line connecting the points for the same participant
    plot([stdDevsData(i, 1), stdDevsData(i, 3)], [peakPowers1(i), peakPowers3(i)], 'k-');
end
...
% add line from DELTA plot
% plot(xFitNback, yFitNback, '-r', 'LineWidth', 2);

% Show the plot
hold off;

saveas(gcf, '/Volumes/methlab/Students/Arne/MA/figures/gaze/Nback_gaze_variability_raw.png');
...
function d = cohensD(data1, data2)
    ...
    s_pooled = sqrt(((n1 - 1) * var(data1) + (n2 - 1) * var(data2)) / (n1 + n2 - 2));
...
%% Function to find peak alpha power, standard deviation, SEM, and frequency (N-back Task)
